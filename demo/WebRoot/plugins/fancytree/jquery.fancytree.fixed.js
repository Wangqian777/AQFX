...
//		}
       ...
       //off是后来添加的
		}).off("click", ".fancytree-fixed-wrapper-bl .fancytree-expander").on("click", ".fancytree-fixed-wrapper-bl .fancytree-expander", function (evt) {
			var $trLeft = $(this).closest("tr"),
				$trRight = $trLeft.data("fancytree-node-counterpart"),
				node = $.ui.fancytree.getNode($trRight),
				rootCtx = $.extend({}, ctx, {node: node});
			_this.nodeSetExpanded(rootCtx, !node.expanded);
...
//			fixedWidth = Math.min(wrapperWidth, Math.max($topLeftTable.outerWidth(), $bottomLeftTable.outerWidth())),
		...
		$topRightWrapper.css({
			width: wrapperWidth - fixedWidth - (true ? 17 : 0),
			height: fixedHeight,
			left: fixedWidth
		});
		$bottomLeftWrapper.css({
		   ...
		   //height: vScrollbar ? wrapperHeight - fixedHeight : "auto",
			height: vScrollbar ? wrapperHeight - fixedHeight - (true ? 18 : 0) : "auto",
			top: fixedHeight
		});
	    ...
	    //20170518添加内容 start
		for (var m = 0; m < $("tbody tr", $bottomLeftWrapper).length; m++) {
		    var __bottomlefttd = $($("tbody tr", $bottomLeftWrapper)[m]).find("td");
		    var __bottomlefttdlength = $($("tbody tr", $bottomLeftWrapper)[m]).find("td").length;
		    if (__bottomlefttdlength > 0) {
		        for (var i = 0; i < __bottomlefttdlength; i++) {
		            __bottomlefttd.eq(i).css({
		                width: $("thead tr th", $topLeftWrapper).eq(i).width(),
		                "min-width": $("thead tr th", $topLeftWrapper).eq(i).width()
		            })
		        }
		    }
		}

		for (var m = 0; m < $("tbody tr", $bottomRightWrapper).length; m++) {
		    var __bottomrighttd = $($("tbody tr", $bottomRightWrapper)[m]).find("td");
		    var __bottomrighttdlength = $($("tbody tr", $bottomRightWrapper)[m]).find("td").length;
		    if (__bottomrighttdlength > 0) {
		        for (var i = 0; i < __bottomrighttdlength; i++) {
		            __bottomrighttd.eq(i).css({
		                width: $("thead tr th", $topRightWrapper).eq(i).width(),
		                "min-width": $("thead tr th", $topRightWrapper).eq(i).width()
		            })
		        }
		    }
		}

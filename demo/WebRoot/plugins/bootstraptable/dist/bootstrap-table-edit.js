...
(function ($) {
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        $.each(that.columns, function (indx, col) {
            ...
            } else if (col.edit && col.edit.type == 'tree') {
                if (col.edit.data.length > 0) {
                    that.$body.find('>tr').each(function () {
                        if (that.getData().length < 1) return true;
                        var rowData = that.data[$(this).data('index')];//当前点击td所在行的数据
                        var $td = $(this).find('td').eq(col.fieldIndex);
                        $.each(col.edit.data, function (i, data) {
                            if (data["text"] == rowData[col.field]) {
                                $td.html(data["text"]);
                            }
                        });
                    });
    ...
    BootstrapTable.prototype.initEdit = function () {
        ...
        $(document).bind('mousedown', function (event) {
            ...
            if (!($target.parents().andSelf().is(that.$body)) && !($target.parents().andSelf().is($('.datetimepicker')))) {
                //console.log($(that.$body.parents(".bootstrap-table")[0]).parent("div").parent("div"));
                var __length_boxbody = $target.closest(".msc_box_body").length;
                var __length_layuitab = $target.closest(".layui-tab-content").length;
                var __length_idps = $target.closest("#PaymentSchedule").length;
                var __length_idpd = $target.closest("#PaymentDetails").length;
                if (__length_idps > 0 || __length_idpd > 0) {
                    if (__length_boxbody > 0) {
                        // if($target.closest($(that.$body.parents(".bootstrap-table")[0]).length>0)){
                        setTimeout(function () {
                            recover(that);
                    ...
                    if (__length_layuitab > 0 && __length_boxbody == 0) {
                        if ($target.closest($(that.$body.parents(".bootstrap-table")[0]).parent("div").parent("div")).length > 0) {
                            setTimeout(function () {
                                recover(that);
                                //that.prevEditRow = null;
                                //that.$body.find('> tr').removeClass('editable-select');
    ...
    function initTrClick(that, _this) {
        ...
        if (!tdOpt.edit.type || tdOpt.edit.type == 'text') {
            ...
            $td.html('<div style="margin:0;padding:0;overflow:hidden;border:solid 0px red;height:' + (height) + 'px;width:' + (width) + 'px;" class="input-group date form_datetime" data-link-field="dtp_editable_input">'
		            	+ '<input class="form-control" placeholder="请选择..." type="text" value="' + value + '" style="height:' + (height) + 'px;">'
        ...
        } else if (tdOpt.edit.type == 'tree') {
            $td.html('<div style="margin:0;padding:0;  position:static;  overflow:hidden;border:solid 0px red;height:' + (height) + 'px;width:' + (width) + 'px;" class="input-group" >'
		            	+ '<input class="form-control"  placeholder="请选择..." type="text" value="' + value + '" style="height:100%">'
		            	+ '<span class="input-group-btn" style="position:static;"><button class="btn btn-primary btn-sm dropdown-toggle"  data-toggle="dropdown" style="height:100%" type="button">'
                         + '<span class="caret"></span></button>'
                    + '<div class="dropdown-menu" style="width:' + (!tdOpt.edit.width ? 0 : parseInt(tdOpt.edit.width)) + 'px;height:' + (!tdOpt.edit.height ? 0 : parseInt(tdOpt.edit.height)) + 'px;  padding:0;margin:0;  overflow:auto">'
						+ '</div>'
                        + '</span>'
					+ '</div>'
                    + '<input type="hidden" id="dtp_editable_input" value="' + value + '"/>'
            ...
            var $Tree = $td.find(".dropdown-menu");//Tree对象
            var $editTr = $(".editable-select");
            var $table_container = $(".fixed-table-container");
            $Tree.css({
                top: ($editTr.position().top + $td.height()),
                left: $editTr.position().left
            });
            $Tree.treeview({
                data: tdOpt.edit.data,
                levels: 1,
                //color: "#4f8fc1",
        ...
        } else if (tdOpt.edit.type == 'pop') {
            $td.html('<div style="margin:0;padding:0;  position:static;  overflow:hidden;border:solid 0px red;height:' + (height) + 'px;width:' + (width) + 'px;" class="input-group" >'
		            	+ '<input class="form-control"  placeholder="请选择..."  type="text" value="' + value + '" style="height:100%">'
		            	+ '<span class="input-group-btn"><button class="btn btn-primary btn-sm" style="height:100%" type="button">'
